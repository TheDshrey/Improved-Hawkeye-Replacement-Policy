...
namespace knob
{
    ...
    extern uint32_t num_partitions = 6;
    vector <uint32_t> partition_size{64 , 64 , 64 , 64 , 64 , 64};
}

void HawkeyeRepl::init_knobs()
{
    knob::hawkeye_max_rrip = 15;
}
...
void HawkeyeRepl::initialize_replacement()
{
    ...
    // Allocate RRIP and metadata arrays for each set
    for (uint32_t set = 0; set < LLC_SET; ++set) {
        // Determine the partition for this set
        uint32_t partition = determine_partition(set);

        // Allocate RRIP array for the partition
        rrip[set] = (uint32_t*)calloc(knob::partition_size[partition], sizeof(uint32_t));
        assert(rrip[set]);

        // Allocate metadata for the partition
        metadata[set] = (HawkeyeMetadata**)calloc(knob::partition_size[partition], sizeof(HawkeyeMetadata*));
        assert(metadata[set]);

        // Initialize metadata for each way in the partition
        for (uint32_t way = 0; way < knob::partition_size[partition]; ++way) {
